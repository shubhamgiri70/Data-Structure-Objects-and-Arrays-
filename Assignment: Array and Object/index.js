...
for (let movie of movies) {
  for (let genre of movie.genres) {
    allGenres.push(movies.genres);
  }
}

// 33. store the actors of all the movies in an array name `allActors`

let allActors = [];

for (let movie of movies) {
  for (let actor of movie.actors) {
    allActors.push(movies.actors);
  }
}

// 34. log the lenght of `allTitles`, `allGenres` and `allActors`
